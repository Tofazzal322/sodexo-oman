...
const InspiredThinking = () => {
  ...
  return (
    <div className="mt-5 container mb-5 inspired-container">
      <h1 className="mt-5 title-1 inspired-container"> Inspired Thinking {inspired.length} </h1>

          <Carousel className="mt-4 inspired-carousel " activeIndex={index} onSelect={handleSelect}>
            ...
            {inspired.slice(4,8).map(item => (
              <Col>
                <Card className="inspired-card">
                  ...
                  <Card.Body>
                     <p > <span className="subcategory-text">{item.subCategory}</span> <small className="text-danger">{item.date}</small> </p>
                     <Card.Title className="title-1"> <Link className="text-decoration" to="/news">{item.title}</Link>  </Card.Title>
                  </Card.Body>
                </Card>
              </Col>
            ))}
